...
window.title("AD1E")
admin = {"adie123": "123"}
login_names = {"test": "test", "again": "again"}
...
def admin_com():
    ...
    label_title = CTkLabel(loginwindow, text=f"Welcome, Admin", font=("Comic Sans MS",25))
    label_title._set_appearance_mode("dark")
    label_title.place(x=20,y=20)
    def ban_command():
        user_ban = ban_username.get()
...
def guest():
    ...
    loginwindow.geometry("400x500")
    loginwindow._set_appearance_mode("dark")
    loginwindow.title("Guest")
            
    label_title = CTkLabel(loginwindow, text=f"Welcome, Guest", font=("Comic Sans MS",25))
    label_title._set_appearance_mode("dark")
    label_title.place(x=20,y=20)
        
    role = CTkLabel(loginwindow, text="You are the Guest!", font=("Comic Sans MS",46))
    role._set_appearance_mode("dark")
    role.place(y=300)
...
def button_login():
    username = login_entry.get()
    password = login_password.get()
    if username in login_names and login_names[username] == password:
        guest()
        
    elif username in admin and admin[username] == password:
        admin_com()
        
    elif username not in login_names and KeyError:
        label_incorrect = CTkLabel(window, text="Incorrect Password and Username.", font=("Comic Sans MS",15))
        label_incorrect._set_appearance_mode("dark")
        label_incorrect.place(x=60,y=280)
